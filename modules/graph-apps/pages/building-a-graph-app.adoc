...
. Knowledge of the xref:ROOT:javascript.adoc[offical JavaScript Drivers]
...
There is plenty of content over on the xref:ROOT:javascript.adoc[JavaScript developer guides] so we will not cover this in too much detail, only the nuances around how the driver works with the Neo4j Desktop API.
...
====
[NOTE]
It is recommended that you use the latest version of the Driver, which is currently **4.0.2**.  You can find full installation instructions on the xref:ROOT:javascript.adoc[JavaScript language guide].
====
...
Additionally, you can specify a command and argument to automatically run as it loads.  For example, if you wanted to run the `:play movies` command to open the Movies xref:ROOT:guide-create-neo4j-browser-guide.adoc[Browser Guide], you could use the following link:
...
For example, if your Graph App requires xref:ROOT:neo4j-apoc.adoc[APOC] then your `manifest.json` file may look something like this:
...
For example, you could create a xref:ROOT:neo4j-browser.adoc#browser-guides[Browser Guide] to explain your project to your coworkers or create set of link:/developer/cypher-query-language/[Cypher scripts] to seed a new database within the project or to hold commonly run queries.
...
For example, a visualisation app may take a set of cypher queries and display them in a xref:ROOT:tools-graph-visualization.adoc[forced graph layout].
