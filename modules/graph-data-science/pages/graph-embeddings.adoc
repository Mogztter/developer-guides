...
| xref:random-projection[Random Projection] | Fast |   No | Linear Algebra
| xref:node2Vec[node2Vec] |  Intermediate |  No | Neural Network
| xref:graph-sage[GraphSAGE] | Slow |   Yes | Neural Network
|===
